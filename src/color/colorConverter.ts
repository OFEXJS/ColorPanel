...
/**
 * 将HEX转换为RGB格式
 * @param hex HEX颜色字符串 (支持#RGB和#RRGGBB格式)
 * @returns RGB颜色对象
 */
export const hexToRgb = (hex: string): RGB | null => {
  // 支持3位和6位HEX格式
  const result = /^#?([a-f\d]{3})$/i.exec(hex);
  if (result) {
    // 3位HEX格式: #RGB -> #RRGGBB
    const r = parseInt(result[1][0] + result[1][0], 16);
    const g = parseInt(result[1][1] + result[1][1], 16);
    const b = parseInt(result[1][2] + result[1][2], 16);
    return { r, g, b };
  }
